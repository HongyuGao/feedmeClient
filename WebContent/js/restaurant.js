...
function dishAppend(dish) {
  $("#dishes").append(
    ...
    "<a href='#' class='btn btn-success btn-sm pull-right' role='button'" +
    " onclick='addToCart(" + '"' + dish.name.trim() + '"' + ", " + dish.id +
    ", " + dish.price + ")'>" +
...
function setRestaurantInfo(restaurant) {
  ...
  $("#restaurantInfo").append(
    "<li" + ' style="text-align: left" ' + "> " + "Name: " + restaurant.name +
    "</li>" + "<br>" +
    "<li" + ' style="text-align: left" ' + "> " + "Phone: " + restaurant.phone +
    "</li>" + "<br>"
  );
}
...
function updateOrder(flag, name, id, price) {
  ...
  if (flag == 1) { // if the dish is already in the cart:
    ...
    if (new_quantity >= 0) {
      ...
      document.getElementById(id + "price").innerHTML = dish_total_price +
        price;
    ...
    $("#dish_info").append(
      '<li class="dish_item"> ' +
      name +
      '<button type="button" onclick="updateOrder(' + 1 + ", '" + name +
      "', " + id + ', ' + (-1.0 * price) + ')"' +
      ...
      '<button type="button" onclick="clearDishItem(' + id + ",'" + name.trim() +
      "', " + (-1.0 * price * order[name]["count"]) + ')"' + '>' +
      ' X </button>' +
      ' </li>'
    );
  }
