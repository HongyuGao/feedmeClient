...
function appendDish(dish) {
  $(".dishes").append(
    '<div class="dish" onclick="pop(\'' + PICTURE_HOST + '/img/photo/' + dish
    .photo + '\', \'' + dish.name + '\', ' + dish.id + ', ' + dish.price +
    ')">' +
...
function addToCart(name, id, price, quantity) {
  ...
  if (flag == 0) {
    ...
    order[restaurantId][name]["count"] = Number(order[restaurantId][name][
      "count"
    ]) + quantity;
  ...
  for (var restId in order) {
    ...
    for (var dishName in dishes) {
      if (dishes[dishName]["count"] != 0) {
        emptyItem = false;
        item += ("," + dishName + "," + dishes[dishName]["id"] + "," + dishes[
          dishName]["price"] + "," + dishes[dishName]["count"]);
      }
    }
    item += "#";
    if (emptyItem == true) {
